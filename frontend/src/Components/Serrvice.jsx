...
const Serrvice = () => {
  ...
  return (
    <>
      <div
        id="service"
        className="min-h-fit flex my-6 py-8 justify-evenly items-center "
      >
        {servieData.map((em, idx) => (
          <div
            key={idx}
            className="min-h-[300px] flex justify-center items-center flex-col   min-w-fit"
          >
            <button className="text-5xl text-stone-800 bg-gray-300 mb-4 p-4 rounded-full ">
              {em.icons}
            </button>
            <p className=" text-slate-700 font-extrabold">{em.seviceName}</p>
